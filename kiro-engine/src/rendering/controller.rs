...
use crate::rendering::owned_data::{OwnedData, Ref};
use crate::rendering::renderer_plan::{RenderNode, RenderPlan};
...
impl Controller {
  ...
  fn get_processor_ref(&self, key: ProcessorKey) -> Result<Ref<BoxedProcessor>> {
    self
      .processors
      .get(key)
      .ok_or(Error::ProcessorNotFound(key))
  }
  ...
  fn get_parameter_value(&self, param_key: ParamKey) -> Result<Arc<ParamValue>> {
    self
      ...
      .get(param_key)
      .cloned()
      .ok_or(Error::ParamValueNotFound(param_key))
  }
  ...
  fn get_audio_buffer_ref(&self, key: AudioBufferKey) -> Result<Ref<AudioBuffer>> {
    self
      .audio_buffers
      .get(key)
      .ok_or(Error::AudioBufferNotFound(key))
  }
  ...
  pub fn get_event_buffer_ref(&self, key: EventsBufferKey) -> Result<Ref<EventsBuffer>> {
    self
      .event_buffers
      .get(key)
      .ok_or(Error::EventsBufferNotFound(key))
  }
