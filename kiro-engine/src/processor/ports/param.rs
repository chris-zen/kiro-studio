...
#[derive(Debug, Clone)]
pub enum ParamData {
  FromValue(Arc<ParamValue>, Ref<AudioBuffer>),
  FromOutput(Ref<AudioBuffer>),
}

impl ParamData {
  pub fn from_value(value: Arc<ParamValue>, slice_buffer: Ref<AudioBuffer>) -> Self {
    Self::FromValue(value, slice_buffer)
  }

  pub fn from_output(buffer: Ref<AudioBuffer>) -> Self {
    Self::FromOutput(buffer)
  }

  pub fn allocated_buffer_key(&self) -> Option<Key<AudioBuffer>> {
    match self {
